...
export const fetchAllCars = async () => {
	try {
		const { data: primaryApiData } = await axios.get(primaryApi);
		const { data: allCarsResponse } = await axios.get(primaryApiData.routes.allCars);
  ...
  const state: any = store.getState();
	...
	try {
		...
		if (!Object.keys(state.primaryApi).length) {
			const { data: primaryApiData } = await axios.get(primaryApi);
      const { data: mercedesBenzData } = await axios.get(primaryApiData.routes.mercedesBenz);
      return mercedesBenzData;
    }
    
    const { data: mercedesBenzData } = await axios.get(state.primaryApi.primaryApi.routes.mercedesBenz);
    return mercedesBenzData;
